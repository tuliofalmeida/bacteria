...
def multiple_experiments(data_path,format_image, align, model):
    """
    ...
    * SuperSegger -> Omnipose -> SuperSegger 
    ...
    model : int
        Parameter to choose the model, 0 = bact_phase_omni,
        1 = bact_fluor_omni. The bacht_phase_omni model is
        trained using phase contrast images and can work
        well in fluoresence data using inverted images, the
        code already perform the invertion. Also we need to 
        use a higher value for mask parameter (using 2.4),
        this model can work well in experiments were you have
        a big change in fluorescence intensity and/or in focus. 
        The bact_fluor_omni is trained using fluorescence images,
        and work well in experiments with little variation 
        in focus and fluorescence.
        ...
        for xy,_ in enumerate(sub_folders):
            ...
            if os.path.exists(folder_mask[xy]) == False and 'xy' not in ref_log and 'raw' not in ref_log:
                ...
                if os.path.exists(folder_omni[xy]) == False:
                    ...
                    if model == 0:
                        os.system('python -m omnipose --dir %s --save_png --dir_above --no_npy --in_folders --invert --omni --pretrained_model bact_phase_omni --cluster --mask_threshold 2.4 --flow_threshold 0 --diameter 30'%(temp))
                    elif model == 1:
                    ...
                    if model == 0:
                        os.system('python -m omnipose --dir %s --save_png --dir_above --no_npy --in_folders --invert --omni --pretrained_model bact_phase_omni --cluster --mask_threshold 2.4 --flow_threshold 0 --diameter 30'%(folder_omni[xy]))
                    elif model == 1:
...
def multiple_fovs(data_path, format_image, align, model):
    """
...
def one_fov(data_path,format_image, align, model):   
    """
    ...
    * SuperSegger -> Omnipose -> SuperSegger 
    ...
    if os.path.exists(folder_mask) == False and 'xy':
        ...
        print('Starting Omnipose!')
        if model == 0:
            os.system('python -m omnipose --dir %s --save_png --dir_above --no_npy --in_folders --invert --omni --pretrained_model bact_phase_omni --cluster --mask_threshold 2.4 --flow_threshold 0 --diameter 30'%(folder_omni))
        elif model == 1:
...
def help_pipeline():
    ...
    * SuperSegger -> Omnipose -> SuperSegger 
    ...
    GitHub
    -------
    Check the documentation in the pipeline folder in GitHub.
    https://github.com/tuliofalmeida/bacteria/edit/main/pipeline/README.md
    """